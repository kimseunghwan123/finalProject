...
function  updGameMatchBoard(eventStatus){
	...
	if(validation(eventStatus)){
		...
		var boardNo = ${gamematchboardOne.boardNo};
		var mappingUrl;
		var dataObj;
		var rsltMsg;
		...
		if(eventStatus == "DEL"){
			mappingUrl  = "gamematchboard.delInfo";
...
<body>
	...
	<!-- FORM 영역 START -->
	<form id="frm" action="regGameMatchBoard" method="post">
		<table class="table-light table-striped text-center" width="100%">
			<tr>
				<th class="th_left"><span>제목</span></th>
				<td><input type="text" id="title" name="boardTitle" maxlength="26" value="${gamematchboardOne.boardTitle}" style="width:650px;"/></td>
			</tr>
					...
					<div class="radio-btn-wrap" id="gamematchboard_rdo_wrap">
						<span class="radio-btn">
							    ...
							    <c:if test="${!empty gamematchboardOne.status}">
									<c:choose>
									<c:when test="${'Y' eq gamematchboardOne.status}">
										<input type="radio" id="rdo_statusY" name="status" checked="" value="${gamematchboardOne.status}">
									</c:when>
									<c:otherwise>
       ...
       <tr>
            <th><span>내용</span>	</th>
            <td>
            	<c:if test="${not empty gamematchboardOne.boardContent}">
            		<textarea rows="10" cols="30" id="txtArea_content" name="boardContent" onkeyup="countText();"> ${gamematchboardOne.boardContent} </textarea>
            	</c:if>
